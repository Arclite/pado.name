...
override func loadView() {
    #warning("Set an actual view here")
    super.loadView()
...
```
The `init` method is pretty straightforward: just call `super.init(nibName:bundle:)` with `nil` because we aren’t using Interface Builder. Creating our own `init` method cleans up the call-site a bit, and gives us a place to do real initialization later. I use `loadView` here rather than `viewDidLoad` because eventually I’ll be creating my own view subclass to load in here rather than using the view created by the system. For this reason, [Apple doesn’t recommend calling `super` here](https://developer.apple.com/documentation/uikit/uiviewcontroller/1621454-loadview#); that’s one of the first things you should change after the initial project setup.
### App Delegate
